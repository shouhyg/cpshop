...
//    }
    ...
    public function getChildrenGroupIds($withself = false)
    {
        ...
        //如果当前的权限分组中存在一个分组的父及分组是另一个则删除该条记录
        foreach ($groups as $k => $v) {
            if (in_array($v['pid'], $originGroupIds)) {
                //通过array_diff 获得去掉指定的数组剩下的数组
        ...
        foreach ($objList as $k => $v) {
            $childrenGroupsIds[] = $v['id'];
        }

        if (!$withself) {
            //如果不包含自己的组别 则从所有组别中去除当前登录用户所拥有的的角色组
            // $groupIds 为当前登录用户所拥有的的所有的角色组的id,一个用户属于多个角色组的情况
            $childrenGroupIds = array_diff($childrenGroupsIds, $groupIds);
        }
        return $childrenGroupsIds;
    }
